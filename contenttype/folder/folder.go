...
func (f *Folder) StoreContent(rc io.ReadCloser, mb []byte, c contenttype.Changes) ([]string, error) {
	...
	return f.StoreMeta(mb, c)
}

func (f *Folder) StoreMeta(mb []byte, c contenttype.Changes) ([]string, error) {
	var (
		meta   FolderMeta
		hashes []string
	)
...
func (f *Folder) MetaToChanges([]byte) (contenttype.Changes, error) {
...
func (m FolderMeta) ToMetadata(im index.Metadata) {
	if m.Foldername != "" {

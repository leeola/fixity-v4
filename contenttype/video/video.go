...
type VideoStorer struct {
	store store.Store
	index index.Indexer
}

func New(c Config) (*VideoStorer, error) {
	if c.Store == nil {
...
func (f *VideoStorer) StoreContent(rc io.ReadCloser, mb []byte, c contenttype.Changes) ([]string, error) {
	if rc == nil {
...
func (f *VideoStorer) StoreMeta(mb []byte, c contenttype.Changes) ([]string, error) {
	var (
		meta   VideoMeta
		hashes []string
	)
...
func (v *VideoStorer) MetaToChanges([]byte) (contenttype.Changes, error) {
	return nil, errors.New("not implemented")
}

func WriteFileMeta(s store.Store, i index.Indexer, m VideoMeta) (string, error) {
...
func (m *VideoMeta) ApplyChanges(c contenttype.Changes) {
...
func (m VideoMeta) ToMetadata(im index.Metadata) {
	if m.Format != "" {
		im["format"] = m.Format
	}
}
